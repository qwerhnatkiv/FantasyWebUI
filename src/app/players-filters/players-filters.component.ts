...
import {
  ...
  USER_ID_NAME,
  DEFAULT_FORM_LENGTH,
  DEFAULT_FORM_LENGTH_COUNT,
  REMOVE_PLAYERS_WITH_NO_GAMES,
  RESET_ALL_FILTERS_LABEL,
  GO_TO_FANTASY_TEAM_LABEL,
  SHOW_UPSIDE_LINES_LABEL,
...
export class PlayersFiltersComponent implements AfterViewInit {
  ...
  @Output() sendUpperBoundPrice: EventEmitter<number | undefined> =
    new EventEmitter<number | undefined>();
  @Output() sendPositions: EventEmitter<string[]> = new EventEmitter<
    string[]
  >();
  @Output() sendTeams: EventEmitter<string[]> = new EventEmitter<string[]>();
  @Output() sendPowerPlayUnits: EventEmitter<string[]> = new EventEmitter<
    string[]
  >();
  @Output() sendSelectedUser: EventEmitter<string | undefined> =
    new EventEmitter<string | undefined>();
  @Output() sendPlayersAreNotPlayedDisabled: EventEmitter<boolean> =
    new EventEmitter<boolean>();
  @Output() sendHideLowGPPlayersEnabled: EventEmitter<boolean> =
    new EventEmitter<boolean>();
  ...
  showOnlyUpsideLinePlayersChanged() {
    this._filtersObservableProxyService.triggerSelectOnlyFromUpsideLinesSubject(this.showOnlyPlayersInUpsideLines);
  }
  ...
  areDefaultPositions(): boolean {
    return (
      this.positionsFormControl.value?.length === 2 &&
  ...
  public resetAllFilters() {
    ...
    this.showOnlyPlayersInUpsideLines = false;
    this.showOnlyUpsideLinePlayersChanged();
  ...
  public openSportsRuProfile(): void {
    window.open(
      `https://www.sports.ru/fantasy/hockey/team/${this.selectedUserId}.html`,
    );
  }
}
