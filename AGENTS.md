...
| `url-state/view-state-url.service.ts` — `ViewStateUrlService` | Keeps the whole `/main` view state in query params so a copied link reopens the same screen: dates, calendar toggles (minesweeper + its drawing mode, past games, easy series, collapsed calendar), best-by-OFO, players picked by clicking a row, selected user (УЗ) and every players filter. `initialState` is a one-time snapshot of `window.location.search`; `patch()` merges and rewrites the address via `Location.replaceState`. Parsing/serialization live in the exported `parseViewUrlState`/`serializeViewUrlState` (unit-tested in the sibling `.spec.ts`). | Stateful snapshot + address-bar writer |
...
- **View state lives in the URL, and it is read exactly once.** Every toggle and filter on `/main` is mirrored into query params by `ViewStateUrlService` (see Services inventory). Two rules when adding a new filter or toggle: (1) call `patch()` wherever the value changes, *and* restore it from `initialState` where the owning component/service initializes — a param that's only written produces links that silently drop it; (2) the restore is a snapshot, not a subscription — the service never reacts to later address changes, so nothing re-renders when it writes. Restoration order matters: `MainViewComponent`'s constructor reads the snapshot *before* the first `getGames` call (so `form`/`user` don't cause a second round-trip), `PlayersFiltersComponent` fills its own controls without emitting, and `CalendarObservableProxyService`/`FiltersObservableProxyService` seed their `BehaviorSubject`s from it — those are `BehaviorSubject`s rather than plain `Subject`s precisely so `calendar-table`/`players-table`, which subscribe later than `header-menu`/`players-filters` publish, still get the restored value.
- **Programmatic `mat-select` changes must be filtered out of `(onSelectionChange)` handlers** — `PlayersFiltersComponent.positionOnClick` guards on `event.isUserInput` because it calls `setValue()` itself for the goalie case; without the guard, restoring `pos=GK` from a link recursed until `Maximum call stack size exceeded`.
